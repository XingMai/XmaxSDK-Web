...
插帧接管后，原帧与中间帧统一由画布显示，并按媒体时间戳保持单调前进。首次接管或恢复时
从当前原帧接续，不能回放已经被原视频越过的旧帧。跳帧、短帧间隔及 GPU 忙碌时仅跳过插值，
不临时隐藏画布；没有后续输入时显示最后已捕获的原帧。中间帧按 `expectedDisplayTime` 调度，
超时、帧对失效或时间戳落后于已显示画面时直接丢弃。关闭、真正故障、尺寸不匹配或后台重置才交回原视频。
...
- 可加 `&jitter=1` 以 20–80ms 的不均匀间隔提交输入；页面展示实际提交的原帧/插帧数量及调度时间戳回退次数。
  这些计数只用于验证页，不新增 SDK 公共状态或逐帧日志，也不代替真实视频的视觉验收。
- 2026-09-21：使用当前自训模型，在内置浏览器 WebGPU 验证 320×180 稳定输入、1248×702 不均匀输入，
  均输出中间帧且未记录到提交时间戳回退或 GPU 错误；关闭/重新开启及卸载/重挂通过。
